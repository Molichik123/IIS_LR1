...
Добработка сервиса предсказаний.
-----
Цель:
...
Чтобы протестировать сервис предсказаний был создан файл *requests.py*, в котором через случайные промежутки времени будут отправляться запросы к основному сервису предсказаний.

*compose.yml* хранит описание сборки проекта, состоящего из сервиса предсказаний и сервиса отправки в него запросов.
Через *docker compose* можно собрать проект 
